...
int main() {
    ...
    // Function/Drivers to play soound in the game
    SoundBuffer gameoversound, death;
    Sound sound;

    // gameover.ogg and die.ogg by blaz125, https://www.sounds-resource.com/nes/supermariobros/sound/276/
    // license: Free for personal use
    // No copyright infringement intended

    // Loads the gameover.ogg into gameoversound SoundBuffer
    if (!gameoversound.loadFromFile("sounds/gameover.ogg")) {
        return EXIT_FAILURE;
    }


    // Loads the die.ogg into death Sound Buffer
    if (!death.loadFromFile("sounds/die.ogg")){
        return EXIT_FAILURE;
    }

    // Function/Driver to display font on screen
    Font font1;

    // orangejuice.ttf by Brittney Murphy, https://www.1001freefonts.com/orange-juice.font
    // license: Free for personal use
    // No copyright infringement intended

    // Loads the orangejuice.ttf into font1 variable
    if (!font1.loadFromFile("fonts/orangejuice.ttf")) {
        return EXIT_FAILURE;
    }
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(e)) {
            // Closes the game if user input is to Interact the game
        ...
        if (timer > delay) {
            ...
            if (grid[y][x] == 2) {
                Game = false;
                sound.setBuffer(death); // Death sound if player snake cut it's self
                sound.play();
            }
            if (grid[y][x] == 0) {
                grid[y][x] = 2;
        ...
        if (grid[y][x] == 1) {
            ...
            // Following loop is to Interact in space where there is no enemy
            for (int i = 0; i < enemyCount; i++) {
        ...
        for (int i = 0; i < enemyCount; i++) {
            if (grid[a[i].gety() / ts][a[i].getx() / ts] == 2) {
                Game = false;
                sound.setBuffer(gameoversound); // Gameover sound if the game is over
                sound.play();
            }
        }

        // Following loop counts the number of tile that has been turn to 1 by the player
        // count variable represent the score of the player playing 1 tile = 1 point
        int count = 0;
        for (int i = 1; i < M-1; i++){
            for (int j = 1; j < N-1; j++){
                if (grid[i][j] == 1){
        ...
        // Following code displays the score while the game is being played by the player
        Text text;
        text.setFont(font1);    // Sets the font
        text.setFillColor(Color::Yellow);   // Sets the font color to yellow
        text.setPosition(540, 380);     // Set the position of the score at bottom right corner
        text.setString("Score " + std::to_string(count));   // Convert int to string to display on screen
        text.setCharacterSize(40);      // Sets the font size
        window.draw(text);      // Display driver/function
